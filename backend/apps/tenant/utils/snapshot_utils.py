...
def getSnapshotList(request):
    snapshots = Snapshot.objects.select_related('sadc').filter(sadc=request.user.sadc)
...
def getSnapshotDetail(request, pk):
    ...
    snapshot = get_object_or_404(Snapshot.objects.select_related('sadc'), id=pk)
...
def getRecentSnapshots(request):
    ...
    snapshots = Snapshot.objects.select_related('sadc').filter(
