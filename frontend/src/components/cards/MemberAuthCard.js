...
const MemberAuthCard = ({ id, data, onEdit }) => {
    ...
    return (
        <CardMember
            title={t('member.authorizations.label')}
            data={auth}
            emptyMessage={t('member.authorizations.no_authorization')}
            onEdit={handleEdit}
            editKey="authorizations"
        >
            ...
            {auth.services?.filter(service => service.id).map(service => (
